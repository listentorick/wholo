...
The organisation-type gate was chosen instead because it is decided entirely from data Wholo already owns and cannot be gamed by choosing a different Keycloak client: every `Membership` row in the system was, when this ADR was written, created by exactly one of two code paths (a third, `StaffInvitationAcceptService.accept`, was added by ADR-067 — always on a `DISTRIBUTOR`-type organisation, so this gate's reasoning is unchanged) — `PortalInvitationsService.acceptInvite` (`role: TRADE_CUSTOMER`, always on a `TRADE_CUSTOMER`-type `Organisation`) or `DistributorsService.createForIdentity` (`role: DISTRIBUTOR_ADMIN`, always on a newly created `DISTRIBUTOR`-type `Organisation`). No other code path creates a `Membership`. A caller's organisation type is therefore an authoritative, DB-resolved fact, not an inference from token provenance.
