...
- **The marker column must be maintained by a Postgres trigger (`BEFORE INSERT OR UPDATE`), never by application code.** Hand-setting it in service methods is not bypass-proof — a raw insert, script, or future code path that forgets to set it silently defeats the uniqueness rule (this happened once; caught by an integration test that inserts directly via raw Prisma specifically to prove the constraint holds independent of app-level code — that test pattern is deliberate, keep it). Triggers are a sanctioned exception to "no hand-edited migration SQL": verified against Prisma's docs/issue tracker that triggers have zero footprint anywhere in Prisma's tooling (no schema DSL, no preview feature, no introspection/diffing awareness at all), so they can never cause the drift this ADR eliminates for indexes.
- **Fact tables are Timescale hypertables — `create_hypertable` is the other sanctioned exception to "no hand-edited migration SQL".** Append-only fact/event-log tables (`order_facts`, `order_line_facts`, `delivery_facts`, and future ones) are hypertables partitioned on `occurredAt`. Prisma has no hypertable concept, so the migration is exactly what `prisma migrate dev --create-only` generated **plus that one appended statement**: `SELECT create_hypertable('<table>', 'occurredAt', create_default_indexes => false);`. Rules that keep it drift-free (ADR-052): the primary key must include `occurredAt` (Timescale requires every unique index to contain the partitioning column), declare `@@index([occurredAt])` in `schema.prisma` so Prisma knows about that index, and pass `create_default_indexes => false` so Timescale does not add a second, undeclared one. Afterwards confirm `prisma migrate diff` reports no difference. Do not append anything else by hand.
