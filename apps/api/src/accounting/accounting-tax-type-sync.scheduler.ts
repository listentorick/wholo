...
// pattern as AccountingContactSyncScheduler/AccountingProductSyncScheduler:
// plain @Interval tick + re-entrancy guard, writing one
// AccountingTaxTypeSyncRequested outbox event per CONNECTED connection.
// Scheduled and manual "sync now" both go through the outbox uniformly — see
// queue.constants.ts for why.
@Injectable()
export class AccountingTaxTypeSyncScheduler implements OnModuleInit {
  private readonly logger = new Logger(AccountingTaxTypeSyncScheduler.name);
  ...
  async requestSyncForActiveConnections(): Promise<void> {
    ...
    for (const connection of connections) {
      try {
        ...
        this.logger.error(`Failed to request scheduled tax type sync for connection ${connection.id}: ${message}`);
      }
    }
  }
}
