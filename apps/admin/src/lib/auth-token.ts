...
/**
 * Centralised admin auth token.
 *
 * Every authenticated admin API request obtains its bearer through `getAuthToken()`
 * (wired into `@wholo/admin-api-client`'s `apiFetch` via `setTokenProvider`). Before
 ...
 * what lets a tab recover after being suspended past the 15-minute access-token
 * lifetime.
 *
...
/**
 * The single source of bearer tokens for authenticated admin requests.
