...
const STATUS_META: Record<AccountingTaxTypeStatus, { label: string; tone: StatusTone }> = {
  ...
  INACTIVE: { label: 'No longer active', tone: 'gray' },
...
const COLUMNS = ['Xero tax rate', 'Rate', 'Matched tax type', 'Match reason', 'Status', 'Actions'];
  ...
  if (taxTypes.length === 0) {
    return (
      <ListEmptyState
        ...
        title={hasFilter ? 'No matching tax rates' : 'No tax rates synced yet'}
        description={hasFilter ? 'Try adjusting or clearing your filters.' : `Click Sync now to pull tax rates from ${providerLabel}.`}
      />
    );
  }
