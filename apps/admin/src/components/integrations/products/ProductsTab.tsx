...
function buildSelectionFilter(filters: ActiveFilter[]): { status?: AccountingProductListParams['status']; type?: AccountingProductListParams['type'] } {
  const filter: { status?: AccountingProductListParams['status']; type?: AccountingProductListParams['type'] } = {};
  for (const f of filters) {
    const values = Array.isArray(f.value) ? f.value : [f.value];
