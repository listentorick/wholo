...
const ASSET_TYPE = 'distributor-banner';
...
export function BrandingBannerUploader({ token, distributorId }: Props) {
  ...
  const bannerUrl = image?.variants['mobile'] ?? null;
  const dominantColor = image?.dominantColor ?? null;

  return (
    <div className="space-y-5">
      {/* Preview */}
      <div className="relative w-full overflow-hidden rounded-lg border border-border bg-[hsl(var(--color-border)/20%)]" style={{ aspectRatio: '3 / 1' }}>
        {loading ? (
          <div className="h-full w-full animate-pulse bg-[hsl(var(--color-border)/30%)]" />
        ) : bannerUrl ? (
          <>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={bannerUrl} alt="Banner" className="h-full w-full object-cover" draggable={false} />
            {dominantColor && (
              <div className="absolute bottom-2 right-2 flex items-center gap-1.5 rounded-full bg-black/50 px-2 py-1 text-[10px] font-medium text-white backdrop-blur-sm">
                <span className="h-2.5 w-2.5 rounded-full border border-white/40" style={{ background: dominantColor }} />
                {dominantColor}
              </div>
            )}
            <button
              ...
              className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-white/90 text-text shadow-sm transition-colors hover:text-red-600"
        ...
        {uploading ? (
          ...
          <>
            ...
            <p className="text-sm text-muted">
              {isDragOver ? 'Drop to upload' : image ? 'Replace banner' : 'Upload banner'}
            </p>
            <p className="text-xs text-muted/70">JPG · PNG · WebP · up to 10 MB · recommended 1920×480</p>
          </>
        )}
