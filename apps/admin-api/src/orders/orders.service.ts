...
export class OrdersService {
  ...
  async acceptOrder(orderId: string, distributorId: string, acceptedByUserId: string) {
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await this.writeOutboxEvent(tx, 'Order', orderId, 'OrderAccepted', {
        orderId,
        distributorId,
  ...
  async rejectOrder(orderId: string, distributorId: string, rejectedByUserId: string, reason: string) {
    const order = await this.prisma.order.findFirst({
      where: { id: orderId, distributorId },
      select: { id: true, status: true, traderCustomerId: true, orderNumber: true },
    });
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await this.writeOutboxEvent(tx, 'Order', orderId, 'OrderRejected', {
        orderId,
        distributorId,
        traderCustomerId: order.traderCustomerId,
        orderNumber: order.orderNumber,
  ...
  async cancelOrder(orderId: string, distributorId: string, cancelledByUserId: string, reason: string) {
    const order = await this.prisma.order.findFirst({
      where: { id: orderId, distributorId },
      select: { id: true, status: true, traderCustomerId: true, orderNumber: true },
    });
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await this.writeOutboxEvent(tx, 'Order', orderId, 'OrderCancelled', {
        orderId,
        distributorId,
        traderCustomerId: order.traderCustomerId,
        orderNumber: order.orderNumber,
