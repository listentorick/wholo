...
### Neutral
- **Deep Navy** (`hsl(217 68% 14%)` / #0B1D3A): primary text color, and — doing double duty — the desktop/mobile sidebar's dark shell background. The same value grounds both text and the one deliberately dark surface in an otherwise light system.
...
`PageShell` is the single page-container primitive and owns four width modes, chosen per page intent rather than by device: **narrow** (480px, `max-w-[480px]`) is the "commerce shell" for order detail and product detail; **full** (no cap) is used by every top-level content page — the home / supplier-list view, the distributor About page, the product catalogue, settings, and the orders views — each managing its own grid and gutters so the page fills the space beside the sidebar edge to edge; **reading** (768px, `max-w-3xl`) and **wide** (896px, `max-w-4xl`) are centred reading columns kept for prose-led pages but currently unused. Default page padding is 20px (`p-5`) on all sides; full-bleed pages opt out and manage their own spacing.
