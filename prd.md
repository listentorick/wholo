...
Wholo is the source of truth for all product data.
...
* flexible metadata attributes (governed by the product type's attribute definitions)

## Product Types

Each product belongs to a product type (e.g. Wine, Beer, Spirits, Bread, Cheese, Food, Cleaning Product).

Product types are distributor-specific. Distributors define the product types they sell.

Product types drive:

* the attribute fields available on a product
* the form fields shown during product entry and editing
* the filters and sort options available in the customer-facing catalogue

The platform will provide standard product type templates. Distributors may use these as-is, extend them or create their own.

## Categories

Categories are used for catalogue navigation and organisational grouping only.

Distributors must be able to:

* create categories
* organise categories into a hierarchy
* assign products to categories

A product may appear in more than one category.

Categories do not define product attributes — that is the role of the product type.

## Product Metadata

Products may carry flexible metadata attributes defined by their product type.

Examples:

* Wine: vineyard, vintage, bottle size, grape variety, ABV, region, organic
* Bread: weight, flour type, allergens, shelf life, sliced or unsliced
* Beer: style, ABV, brewery, volume

Attribute definitions are distributor-specific and control:

* which fields appear on product entry and edit forms
* which fields are shown on product cards and product detail pages
* which fields are available as customer-facing catalogue filters
* which fields are available as customer-facing catalogue sort options

Controlled attributes (e.g. wine type, region, grape variety) define a list of allowed options to ensure consistency across products and catalogue filters.

This feature is planned but not included in the initial release. Products in the initial release will support core fields only.
...
## Xero Product Synchronisation

Wholo is the source of truth for all product data. Xero's item list is kept in sync with Wholo.

### Initial import

When a distributor connects Xero, their existing Xero items are imported into Wholo as a one-time bootstrap. This seeds the initial product catalogue.

During import, the distributor must review and confirm the imported products before they are committed. The platform will attempt to infer the product type for each imported item to reduce manual effort during review.
...
After import, distributors can enrich products with additional information:
...
After the initial import, Wholo owns the product catalogue. When products are created, updated or archived in Wholo, the corresponding item in Xero is updated automatically. This ensures Xero's item list remains current for invoice creation.
...
# 6.12 Product Ingestion

Wholo must support importing product data from external sources to seed and maintain the product catalogue.

Wholo is the source of truth for all product data. Ingested data is mapped into Wholo's product model and owned by Wholo once committed.

## Sources

In the initial release, product ingestion supports Xero only. Future releases will support additional sources such as spreadsheets and supplier documents.

| Source | v1 | Notes |
|---|---|---|
| Xero | Yes | Bootstrap import + ongoing Wholo → Xero sync |
| Excel / CSV | Future | Ingestion only — no write-back to source |
| Supplier documents | Future | Ingestion only — no write-back to source |

## Field Mapping

When ingesting from an external source, field mappings define how source fields map to Wholo product fields.

Distributors must be able to:

* configure field mappings for each source
* save and reuse mappings across subsequent imports from the same source

## Product Type Inference

External sources may not include product type information. Where product types cannot be determined directly from the source data, the platform will attempt to infer the product type from the product name and description.

Distributors must be able to confirm or correct the inferred product type during the review step.

## Review Step

Before imported products are committed to the catalogue, distributors must be able to:

* preview the imported products and their mapped fields
* review and correct field mappings
* confirm or correct inferred product types
* assign categories
* identify and resolve validation errors

Distributors can choose to skip the review step for trusted sources where auto-commit is appropriate.

## Import Job Tracking

Import jobs must be tracked with a clear status so distributors can monitor progress and return to review large imports.

Import job statuses must include:
...
After an import is committed, distributors must be able to view a per-record summary showing which products were successfully imported, which failed and why.
